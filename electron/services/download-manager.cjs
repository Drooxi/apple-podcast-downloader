...
const { runDownload, runDownloadFromRss, validatePodcastId } = require("../../core/podcast-downloader.cjs");
...
class DownloadManager {
  ...
  async startFromRss({ rssUrl, senderId, emitLog, emitProgress = () => {}, emitStatus }) {
    if (this.active) throw new Error("Un téléchargement est déjà en cours.");
    ...
    try {
      const result = await runDownloadFromRss({
        outputDir: this.outputDirectory,
        rssUrl,
        signal: controller.signal,
        onLog: emitLog,
